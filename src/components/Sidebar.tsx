    ...
    <aside className="hidden md:flex w-64 shrink-0 flex-col border-r border-slate-200 bg-slate-50/70 backdrop-blur">
      <div className="h-16 flex items-center gap-3 px-5 border-b border-slate-200">
        <BrandLogo
          ...
          alt={marcaNome}
          className="drop-shadow-[0_0_10px_rgba(26,134,230,0.4)]"
        />
        <span className="font-semibold text-lg tracking-tight text-brand-gradient truncate">
          {marcaNome}
        </span>
      </div>

      <nav className="flex-1 overflow-y-auto p-3 space-y-1">
        <NavLinks isAdmin={isAdmin} isSuper={isSuper} />
      </nav>
